...
- 今日資訊的「作息時間」可在起床／就寢之間切換；兩邊草稿互不覆蓋，只有離開時間欄位時才儲存。凌晨 00:00～05:59 的就寢時間會明確記為隔日凌晨，06:00 起視為當日時間。
- 每日統計只保留「今日完成時間」科目圓環；「今日數學頁數」與「本週數學頁數」不再顯示或在背景計算。數學實際頁碼紀錄與教材進度圖仍完整保留，Calendar 建議範圍本身不會被當成實際進度。
- 各科完成時間以圓環圖呈現，環中顯示國／英／數／自／社，指向或點擊後顯示分鐘與占比。
- 教材進度圖由標題列直接在目前頁面跳轉；「我的教材」可按國文／英文／數學／自然切換並勾選教材，進度圖只顯示已勾選項目。
- 「學習總結」提供全頁共用的週／月切換；切換時整頁資料區會平滑淡出、調整高度再淡入，週曆／月曆、科目分配、學習趨勢、個人狀態、上期比較與本期小結會同步使用同一期間。
- 「個人狀態」獨立顯示平均睡眠時間、平均就寢時間、平均起床時間、有效夜晚數、每日睡眠趨勢及與上期比較。睡眠以「前一日就寢＋當日起床」計算，缺少任一時間或超過 18 小時的組合不列入平均；外出、身體不適與疲倦日的有效作息仍可納入。
- 週曆／月曆以圓內深淺呈現學習時間、外圍圓環呈現完成率；亮綠完整外圈代表 100% 完成，並以紅／橘／黃圓心標示身體不適、疲倦與外出。詳細數字只在指向或點擊日期時顯示。
- 外出或身體不適日仍顯示當日實際進度與學習時間，但該日完成率不列入週／月彙總、上期比較與固定小結；學習時間仍正常列入本期統計。
- 科目分配與學習趨勢各占一半欄位；圓環沿用首頁粗細並將科目字放在環上，點入科目後以同科目深淺色顯示各項目占比、比例與時間，明細以四欄先直向排列。
- 科目分配總覽將物理、化學、生物與地科合併為「自然」；點入自然後再以四種顏色與物／化／生／地標註分科，右側保留細科、實際項目、占比與時數。無法辨識細科目的舊自然紀錄仍會保留。
...
- 英文教材分類中的「學測」包含英文寫作測驗與英文文法總複習講義；手動新增與教材進度圖的「我的教材」共用相同分類。
...
- **v171.6.0-public.30**：新增地理、歷史、公民三本《學測週計畫》的獨立識別碼與頁碼地圖；Calendar 先依識別碼辨識，缺少識別碼時才以唯一命中的單元名稱推測科目，並以固定社會科卡片顯示實際頁碼與對應單元。Calendar Prompt 同步加入三科規則與完整對照。
- **v171.6.0-public.29**：強化公開版 OAuth 前端環境邊界，只允許公開的 Google Client ID 進入 Vite；敏感名稱、真實值誤寫範例檔與非 Google 授權導頁都會被驗收阻止，並抽離 Calendar 連線 controller。
- **v171.6.0-public.28**：修正覆蓋式更新後 CI 仍看見未被載入的舊數學統計檔案而誤判失敗；驗收改為確認正式 runtime 沒有匯入或啟動舊模組，實際功能維持移除狀態。
- **v171.6.0-public.27**：移除每日統計中的「今日數學頁數／本週數學頁數」切換項與專屬背景索引，只保留今日完成時間科目圓環；教材實際頁碼與教材進度圖不受影響。
- **v171.6.0-public.26**：新增起床／就寢共用作息欄位與獨立個人狀態總結，支援跨日就寢、睡眠時間、週／月平均與固定比較文字，且不影響學習時間或完成率。
- **v171.6.0-public.25**：修正結算完成率出現時數學頁數面板視覺偏上的問題；動畫容器會填滿同列剩餘高度，使兩個數學頁數畫面在加高後仍垂直置中，內容維持靠左並保留原切換動畫。
- **v171.6.0-public.24**：教材進度圖中的 Azar 英文文法改為每個大章一段，共 14 段；小節完成紀錄仍按實際頁碼累積到所屬章節，不改變 Calendar 卡片與既有資料。
- **v171.6.0-public.23**：修正數學頁數面板更新檔未包含首頁結構的問題；改由既有面板直接負責垂直置中與靠左排列，不再依賴額外內層容器，並保留原本的面板高度與切換動畫。
- **v171.6.0-public.22**：同步個人版的手機連線設定穩定動畫與 Calendar 啟動保護；返回 Tracker 時優先顯示已存紀錄，僅在 Calendar 成功載入後重比排程。另加入生物、化學「新關鍵」完整頁碼地圖及教材進度列。
- **v171.6.0-public.21**：今日與本週數學頁數面板改為真正垂直置中，並維持數字、進度條及百分比共用左側對齊基準。
- **v171.6.0-public.20**：完成全專案 120 個 TypeScript 檔案的可讀性與一致性檢查，統一相對匯入副檔名、移除 typed code 的巢狀三元與重複條件、補齊頂層回傳型別，功能與資料格式不變。
- **v171.6.0-public.19**：更新 Calendar 建議 Prompt 的物理「逆轉勝」內容，移除暫定全書範圍說明，改列 p.3–255 共 37 段已核對的單元、主題與題型頁碼地圖。
- **v171.6.0-public.18**：將「英文寫作測驗」統一移入英文教材的「學測」分類，涵蓋手動新增、英文複習與教材進度圖的「我的教材」。
- **v171.6.0-public.17**：將「英文文法總複習講義」統一移入英文教材的「學測」分類，涵蓋手動新增、英文複習與教材進度圖的「我的教材」。
- **v171.6.0-public.16**：科目分配總覽將物理、化學、生物、地科合併為「自然」，點入後再以四色圓環與物／化／生／地標註顯示，右側保留完整分項；週／月曆圖例同步改為亮綠 100% 外圈並補上三種狀態色說明。
- **v171.6.0-public.15**：外出或身體不適日仍保留每日進度、時間與完成率顯示，但週／月彙總完成率會排除這些日期；學習時間仍照常計入。
- **v171.6.0-public.14**：新增對話式數學 1、2、3A、4A 分冊講義，以及 1～2、3A～4A 複習講義；頁碼提示、教材進度與標準 Calendar 備註共用同一份對照。英文新增項目選單依用途重分為「學測／課外補充」；教材進度圖的「我的教材」同步將數學分為「分冊講義／複習講義」，英文分為「學測／其他」。
- **v171.6.0-public.13**：英文閱讀與英文複習教材選單也移除「新增講義／其他英文項目」分組，全部改為單一教材清單；至此所有教材選單都不再依新增或其他分類，數學只保留功能性的「分冊講義／複習講義」分類。
- **v171.6.0-public.12**：手動新增自然項目的講義選單移除「新增講義／其他講義」分組，依科目直接列出可用教材；數學講義改依涵蓋範圍分為「分冊講義」與「複習講義」，不變更既有教材值與紀錄。
- **v171.6.0-public.11**：依紙本目錄完成物理「逆轉勝」p.3–255 的 37 段頁碼地圖，細分單元、主題與素養導向題型；紀錄卡頁碼提示與教材進度圖改用實際主題邊界，不再顯示暫定全書範圍。
