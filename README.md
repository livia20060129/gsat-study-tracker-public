...
- **v171.6.0-public.22**：同步個人版的手機連線設定穩定動畫與 Calendar 啟動保護；返回 Tracker 時優先顯示已存紀錄，僅在 Calendar 成功載入後重比排程。另加入生物、化學「新關鍵」完整頁碼地圖及教材進度列；既有數學頁數垂直置中、靠左內容與切換動畫保持不變。
